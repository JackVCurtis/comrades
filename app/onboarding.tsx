...
import { markOnboardingCompleted } from '@/modules/onboarding/onboardingState';
import { type OnboardingPermissionStepKey, useOnboardingPermissions } from '@/modules/onboarding/useOnboardingPermissions';
...
function buildTimeline(steps: Record<OnboardingPermissionStepKey, { status: keyof typeof STATUS_LABELS }>): TimelineEntry[] {
  const permissionStatus = [steps.camera.status].every((status) => status === 'granted')
    ? 'completed'
    : [steps.camera.status].some((status) => status === 'denied' || status === 'blocked')
      ? 'failed'
      : [steps.camera.status].some((status) => status === 'requesting')
        ? 'active'
        : 'pending';
