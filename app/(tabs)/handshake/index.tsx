...
function getExchangeStatus(stepIndex: number): {
  label: string;
...
export default function HandshakeScreen() {
  ...
  const [newCounterpartyName, setNewCounterpartyName] = useState<string | null>(
    null,
  );
  ...
  const exchangeStatus = useMemo(
    () => getExchangeStatus(exchangeStepIndex),
    [exchangeStepIndex],
  );
      ...
      <AppCard style={styles.card}>
        ...
        >
          ...
          <View style={styles.flowContent}>
            {stage === "collecting" ? (
              <View style={styles.flowSection}>
                <ThemedText type="subtitle">
                  Share details for this handshake
                </ThemedText>
                <ThemedText>
                  The name entered here is what you provide to the other person.
                  Their provided name is saved after exchange.
                </ThemedText>
            ...
            {stage === "complete" ? (
              <View style={styles.flowSection}>
                ...
                {handshakedProfileSummary ? (
                  <>
                    <ThemedText>
                      Connected with {handshakedProfileSummary.counterpartyName}
                      .
                    </ThemedText>
                    <ThemedText>
                      Name you shared: {handshakedProfileSummary.nameShared}
                    </ThemedText>

                    <View style={styles.mutualConnectionsSection}>
                      <ThemedText type="defaultSemiBold">
                        Mutual connections (1 step)
                      </ThemedText>
                      <ThemedText>
                        You → Jordan Miles →{" "}
                        {handshakedProfileSummary.counterpartyName}
                      </ThemedText>
                      <ThemedText>
                        You → Riley Chen →{" "}
                        {handshakedProfileSummary.counterpartyName}
            ...
            {stage === "idle" ? (
              <AppButton label="Start Handshake" onPress={startHandshake} />
            ) : null}
          </View>
